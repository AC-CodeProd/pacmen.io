module.exports.purge = function purge(s, action) {
  ...
  if the user is not an owner and not in a room:
    ...
    */
  if (players[s.id].inroom) { //user is in a room
    var room = rooms[players[s.id].inroom]; //get room from rooms object
    if (s.id === room.owner) { //given user is the owner of the room
      if (action === 'disconnect') { //handle the disconnect event
        ...
        if(_.contains((room.people)), s.id) {
          for (var i=0; i<room.people.length; i++) {
            players[room.players[i]].inroom = null;
            players[room.players[i]].roomname = null;
          }
        }
        room.people = 0;
        delete rooms[players[s.id].owns]; //delete the room
        delete players[s.id]; //delete user from people collection
        ...
        // utils.sendToSelf(s, 'sendUserDetail', players[s.id]);
        ...
        delete rooms[players[s.id].owns];
        players[s.id].owns = null;
        players[s.id].roomname = null;
        room.people = 0;
        delete chatHistory[room.name]; //delete the chat history
        totalRooms = _.size(rooms);
        // utils.sendToAllConnectedClients(io, 'updateRoomsCount', {count: totalRooms});
      ...
      if (action === 'disconnect') {
        // utils.sendToAllClientsInRoom(io, s.room, 'sendChatMessage', {name: 'ChatBot', message: players[s.id].name + " has left the server."});
        if (_.contains((room.people), s.id)) {
        ...
        if (_.contains((room.people), s.id)) {
          var personIndex = room.people.indexOf(s.id);
          room.people.splice(personIndex, 1);
          players[s.id].inroom = null;
          players[s.id].roomname = null;
          // utils.sendToAllClientsInRoom(io, s.room, 'sendChatMessage', {name: 'ChatBot', message: players[s.id].name + " has left the room."});
          s.leave(room.name);
          // utils.sendToAllConnectedClients(io, 'updateUserDetail', people);
          // utils.sendToSelf(s, 'sendUserDetail', players[s.id]);
        }
  ...
  if (action === 'disconnect') {
    // utils.sendToAllConnectedClients(io, 'sendChatMessage', {name: 'ChatBot', message: players[s.id].name + " disconnected from the server."});
    delete players[s.id];
    totalPeopleOnline = _.size(people);
    // utils.sendToAllConnectedClients(io, 'updatePeopleCount', {count: totalPeopleOnline});
    var o = _.findWhere(sockets, {'id': s.id});
